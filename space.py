...
def ler_highscores(filename):
    ...
    highscores.sort(key=lambda x: x[1], reverse=True) #lambda recebe o x e devolve o x[1]   maneira mais facil de ordenar os highscores através dos pontos(já não me lembro quem me tinha explicado a stor ou bernardo)
...
def verificar_colisoes_player_bullets(state):
    for bullet in state["enemy_bullets"]:
        ...
        if ((x-bx)**2 + (y-by)**2) <= COLLISION_RADIUS**2:
            ...
            terminar_handler() #rever o porquê de isto funcionar, foi apenas uma hunch mas funcionou
    ...
    for bullet in state["player_bullets"]:
        ...
        for enemy in state["enemies"]:
            ...
            if ((x-bx)**2 + (y - by)**2 <= radius**2):
                ...
                state["score"] += 100 #apenas para testar o score
    return
...
if __name__ == "__main__":
    ...
    # Construção inicial
    if loaded:
        print("Tá a funfar") # func(var[0], var[1]) = func(*var)         Nota: assumindo que var tem apenas duas entrada na lista
        ...
        # Recriar balas
        for x, y in loaded['player_bullets']:
            bullet = criar_bala(x, y, "player")
            state["player_bullets"].append(bullet)
        
        for x, y in loaded['enemy_bullets']:
    ...
    while True:
        ...
        if len(STATE["enemies"]) == 0:
             ...
             STATE["score"] += 500 #apenas para testar a porcaria do score
             terminar_handler()
